...
public class MovingSphere : MonoBehaviour
{
    ...
    private Vector3 _lastContactNormal, _lastSteepNormal, lastConnectionVelocity;
    ...
    private void Update()
    {
        ...
        _playerInput.y = Swimming ? Input.GetAxis("UpDown") : 0f;
    ...
    {
        var ballMaterial = normalMaterial;
        var rotationPlaneNormal = _lastContactNormal;
        ...
        else if (!ONGround)
        {
            if (ONSteep)
            {
                _lastContactNormal = _lastSteepNormal;
            }
            else
            {
        ...
        var movement = (_body.velocity - lastConnectionVelocity) * Time.deltaTime;
        movement -=
            rotationPlaneNormal * Vector3.Dot(movement, rotationPlaneNormal);
        var distance = movement.magnitude;
        var rotation = ball.localRotation;
        ...
        var angle = distance * rotationFactor * (180f / Mathf.PI) / ballRadius;
        var rotationAxis =
    ...
    {
        var ballAxis = ball.up;
        var dot = Mathf.Clamp(Vector3.Dot(ballAxis, rotationAxis), -1f, 1f);
        var angle = Mathf.Acos(dot) * Mathf.Rad2Deg;
        var maxAngle = ballAlignSpeed * traveledDistance;
    ...
    private void ClearState()
    {
        ...
        lastConnectionVelocity = _connectionVelocity;
    ...
    private void AdjustVelocity()
    {
        ...
        else if (InWater)
        {
            ...
            acceleration = Mathf.LerpUnclamped(ONGround ? maxAcceleration : maxAirAcceleration, maxSwimAcceleration,
                swimFactor);
        ...
        adjustment.y = Swimming ? _playerInput.y * speed - Vector3.Dot(relativeVelocity, _upAxis) : 0f;
        ...
        if (Swimming)
        {
            _velocity += _upAxis * adjustment.y;
        }
    }
